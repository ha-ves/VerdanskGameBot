...
{
    internal static class GameServerWatcher_OLd
    {
        private static TimeSpan WatchTimeout { get; set; } = TimeSpan.FromSeconds(5);

        private static Dictionary<string, Tuple<Timer,ManualResetEvent>> Watchers { get; set; }
        private static Timer WatcherTimer { get; set; }

        private static Dictionary<string, Mutex> UpdateMutexes { get; set; }
        ...
        private static void QueryGameServersToWatch()
        {
            ...
            WatcherTimer = new Timer(dueTime: TimeSpan.Zero, period: TimeSpan.FromHours(1), state: null, callback: (obj) =>
            {
        ...
        internal static Task<GameServerModel> ChangeGameServer(SocketModal modal)
        {
            ...
            catch (UpdateIntervalInvalidException gpformatexc)
            {
        ...
        internal static Task<GameServerModel> AddGameServer(SocketModal modal, IMessage placeholder = null)
        {
            var customid = CustomID.Deserialize(modal.Data.CustomId);
            ...
            WatcherTimer.Change(TimeSpan.Zero, TimeSpan.FromMinutes(15));
        ...
        /// <exception cref="UpdateIntervalInvalidException">Thrown when update interval time is invalid</exception>
        private static void ParseHostIPGamePort(SocketModal modal, string host_ip, out IPAddress ip, out ushort gameport, out int update_interval)
        {
            ...
            if (!int.TryParse(modal.Data.Components.First(cmp => cmp.CustomId == CustomIDs.UpdateInterval.ToString("d")).Value, out update_interval)) throw new UpdateIntervalInvalidException();
        }
        ...
        public static async void Dispose()
        {
            ...
            {
                await WatcherTimer.DisposeAsync();
                WatcherTimer = null;
            }
